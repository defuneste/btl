...
I found recently that `git` has a [small script](https://git-scm.com/book/id/v2/Appendix-A%3A-Git-in-Other-Environments-Git-in-Bash) that produce a similar function but way better. 
...
My new version including git provided function is now: 

```bash
if [ "$color_prompt" = yes ]; then
	PS1="${debian_chroot:+($debian_chroot)}\[\033[31;35m\]\$(__git_ps1 '(%s)')\033[00m\]:\[\033[01;34m\]\W\[\033[00m\]\$ "
else
    PS1='${debian_chroot:+($debian_chroot)}\u@\h:\w\$ '
fi
...
The function `__git_ps1` is taking an optional argument to format the returned string. I like `()`so I used that. 
